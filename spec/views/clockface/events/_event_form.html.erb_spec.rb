...
module Clockface
  RSpec.describe "clockface/events/_event_form.html.erb", type: :view do
    let(:task) { event.task }
    let(:event) { create(:event) }

    describe "task (name)" do
      let(:section) { page.find(".events-form__form-element--name") }
      ...
      it "defaults the dropdwon to the event's value" do
        task2 = create(:task)
        event.update!(task: task2)
      ...
      context "dropdown is not editable" do
        it "displays the event name" do
          ...
          expect(section).to have_content(event.name)
        end
      end
    end

    describe "enabled" do
      let(:section) { page.find(".events-form__form-element--enabled") }
    ...
    describe "period" do
      let(:section) { page.find(".events-form__form-element--period") }
      ...
      it "displays the section label and the dropdown selectors" do
        ...
        # Period Units
        dropdown_elements =
          section.find(".events-form__form-element--period-units").all("option")
      ...
      it "defaults the period form fields" do
        ...
        selected_units = find_selected_option(
          section.find(".events-form__form-element--period-units")
        )
    ...
    describe "at" do
      let(:section) { page.find(".events-form__form-element--at") }
      ...
      it "displays the section label and dropdown selectors" do
        ...
        # Day of Week
        dropdown_elements =
          section.find(".events-form__form-element--day-of-week").all("option")
        ...
        # Hour
        dropdown_elements =
          section.find(".events-form__form-element--hour").all("option")
        ...
        # Minute
        dropdown_elements =
          section.find(".events-form__form-element--minute").all("option")
      ...
      it "defaults the `at` form fields" do
        ...
        selected_day_of_week = find_selected_option(
          section.find(".events-form__form-element--day-of-week")
        )
        selected_hour = find_selected_option(
          section.find(".events-form__form-element--hour")
        )
        selected_minute = find_selected_option(
          section.find(".events-form__form-element--minute")
        )
      ...
      context "hour or minute is null" do
        it "defaults to the as the '**' option" do
          event.update!(day_of_week: nil, hour: nil, minute: nil)
    ...
    describe "time zone" do
      let(:section) { page.find(".events-form__form-element--time_zone") }
    ...
    describe "if condition" do
      let(:section) { page.find(".events-form__form-element--if_condition") }
      ...
      it "displays the section label and dropdown selector" do
        ...
        expected_options =
          [""] +
          Clockface::Event::IF_CONDITIONS.keys.map do |i|
            Clockface::Event.
      ...
      it "defaults the dropdwon to the event's value" do
        event.update!(if_condition: "last_of_month")
    ...
    describe "form" do
      it "submits to the URL specified by `form_url`" do
        ...
        form = page.find(".events-new__form-container > form")
        expect(form["action"]).to eq("/foo")
      end

      it "displays the submit button" do
        render_partial
        form = page.find(".events-new__form-submit")

        expect(form.find("input[type='submit']")["value"]).
          to eq(t("clockface.events.event_form.submit"))
      end
      ...
      it "displays the cancel button, linking back to events_path" do
        ...
        expect(form).
          to have_link(
            t("clockface.events.event_form.cancel"),
    ...
    def render_partial(opts = {})
      render(
        partial: "clockface/events/event_form",
        locals: {
          event: event,
