  ...
  module Methods
    ...
    def sync_database_events(opts = {}, &block)
      ...
      ::Clockwork::sync_database_events(model: Clockface::ClockworkScheduledJob, every: opts[:every]) do |job|
        if job.enabled?
          logger.info "[Clockface] Running ClockworkScheduledJob id #{job.id} (\"#{job.name}\")"
          begin
            yield(job)
          rescue Exception => e
            # Clockwork supports defining an error_handler block that gets called
            # when an error is raised. But we do our own error handling here
            # so that we can still update the `last_run_at` below
            logger.error "[Clockface] Error while running ClockworkScheduledJob id #{job.id} (\"#{job.name}\") => #{e.message}"
          end

          job.update(last_run_at: Time.zone.now)
        else
          logger.info "[Clockface] Skipping Disabled ClockworkScheduledJob id #{job.id} (\"#{job.name}\")"
        end
      end
    end
  end
